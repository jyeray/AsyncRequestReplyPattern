...
namespace AsyncRequestReply.Client
{
    class Program
    {
        ...
        static async Task Main()
        {
            ...
            var statusUrl = await CallGateFunction(name);
        ...
        public static async Task<string> CallGateFunction(string name) {
            Console.WriteLine();
            Console.WriteLine("Making api call to Gate function");
            var gateHttpResponseMessage = await client.PostAsJsonAsync("yourGateFunctionUrlHereWithAuthCode", new RequestBody{Name = name});
